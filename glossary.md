...
| **OTP** | **One-Time Password** — A temporary verification code used to verify a user's identity. |
| **PostgreSQL** | An open-source relational database management system used to store Bloom's application data. |
| **RBAC** | **Role-Based Access Control** — A security approach that controls access based on a user's assigned role. |
| **Redis** | An in-memory data store used for fast-access data, caching, and other backend operations. |
| **SlowAPI** | A rate-limiting library for FastAPI applications used to control the number of requests from clients. |
| **SMTP** | **Simple Mail Transfer Protocol** — A protocol used to send emails between applications and mail servers. |
| **SQLAlchemy** | A Python SQL toolkit and ORM used by the backend to interact with the database. |
| **UUID** | **Universally Unique Identifier** — A unique identifier used to identify records in the system. |
